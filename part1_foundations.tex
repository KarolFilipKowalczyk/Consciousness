...
The question of consciousness—how and why subjective experience arises from physical processes—has long stood as one of the most profound challenges in science and philosophy \cite{chalmers1995,nagel1974}. While neuroscience has made remarkable progress in mapping the neural correlates of conscious states \cite{koch2016}, and cognitive science has illuminated the functional architecture of the mind, a fundamental explanatory gap remains \cite{levine1983}. Why does information processing in the brain give rise to the felt quality of experience? Why is there "something it is like" to be conscious?
...
\begin{itemize}
\item \textbf{Integrated Information Theory (IIT)} proposes that consciousness corresponds to integrated information, quantified as $\phimax$, which measures the irreducible cause-effect power of a system \cite{tononi2004,tononi2016,oizumi2014}.

\item \textbf{Global Workspace Theory (GWT)} suggests consciousness acts as a broadcast mechanism, making information globally available to specialized cognitive processes \cite{baars1988,baars1997,dehaene2001}.

\item \textbf{Attention Schema Theory (AST)} frames consciousness as the brain's internal model of its own attentional processes, serving control and predictive functions \cite{graziano2013,graziano2019}.

\item \textbf{Quantum Consciousness (Orch-OR)} proposes that quantum processes in neural microtubules give rise to conscious moments through objective reduction \cite{penrose1989,hameroff1996}.
\end{itemize}
...
Our framework begins with a deceptively simple idea from computational complexity theory \cite{sipser2012,arora2009}: instead of modeling cognition as a single Turing machine with infinite memory \cite{turing1936}, we model it as a hierarchy of finite-state machines with exponentially growing resources.
...
There exists no computable function $s: \text{Problems} \to \mathbb{N}$ that always returns the minimal $n$ such that $M_n$ can solve the given problem. This is related to Kolmogorov complexity \cite{kolmogorov1965,chaitin1975}—finding the shortest description of a solution is non-computable.
\end{theorem}
...
\begin{itemize}
\item \textbf{Why there is something it is like:} The internal phenomenology \cite{husserl1991} of resource-constrained collapse
...
\begin{align*}
M_{10} &: 2^{10} = 1,024 \text{ bits (1K bits)} \\
M_{15} &: 2^{15} = 32,768 \text{ bits (32K bits)} \\
M_{20} &: 2^{20} = 1,048,576 \text{ bits (1M bits)} \\
M_{25} &: 2^{25} = 33,554,432 \text{ bits (32M bits)} \\
&\vdots \\
M_n &: 2^n \text{ bits of memory}
\end{align*}

where realistic cognitive systems likely operate with levels ranging from approximately $M_{15}$ (basic perception) to $M_{35}$ (complex reasoning).
